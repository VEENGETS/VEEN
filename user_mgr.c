...
void user_login()
{
	char user_name[33], user_pass[33], user_pass_check[31];
	printf("\n请登录！\n");
	printf("\n          用户名：");
	scanf("%s", user_name);
	//  输入密码时没有回显
	strcpy(user_pass, getpass("          密  码："));
	//  printf("\n          密  码：");
	

	if(is_first_run())  //  如果第一次使用
	{
		...
		while(1)
		{
			strcpy(user_pass, getpass("\n          密  码："));
			strcpy(user_pass_check, getpass("\n再次输入管理员密码："));
